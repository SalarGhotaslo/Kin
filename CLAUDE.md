...
2. **Home** (`HomeTab`) — "Welcome back" header, a mocked local-outbreak alert banner (chickenpox), Family Snapshot (each child + status tag), Quick Actions (Ask AI / Find Doctor / Community), Recent Activity card with "Continue Chat", horizontally-scrolling Daily Insights, a Milestone Tracker progress bar, and a floating action button to Ask AI.
